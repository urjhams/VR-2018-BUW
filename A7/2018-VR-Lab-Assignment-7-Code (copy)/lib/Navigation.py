...
class NavigationManager(avango.script.Script):
    ...
    def my_constructor(self,
        ...
        self.ray_geometry.Material.value.set_uniform("Color", avango.gua.Vec4(0.0,0.0,1.0,1.0))
        ...
        self.intersection_geometry.Material.value.set_uniform("Color", avango.gua.Vec4(0.0,0.5,0.5,0.5))
    ...
    def set_navigation_technique(self, INT):  
        ...
        if self.active_navigation_technique_index == 0: # Steering Navigation
            ...
            self.ray_geometry.Material.value.set_uniform("Color", avango.gua.Vec4(0.6,0.1,0.2,0.3))
            ...
            self.ray_geometry.Material.value.set_uniform("Color", avango.gua.Vec4(0.0,0.1,0.5,0.5))
            self.intersection_geometry.Material.value.set_uniform("Color", avango.gua.Vec4(0.0,0.0,1.0,1.0))           
...
class TeleportNavigation(NavigationTechnique):
    ...
    def evaluate(self): # implement respective base-class function
        ...
        #print (self.sf_pointer_button.value)
        ...
        if self.NAVIGATION_MANAGER.pick_result is None:
            self.NAVIGATION_MANAGER.update_ray_visualization()
            target = self.NAVIGATION_MANAGER.intersection_geometry.WorldTransform.value.get_translate()
            print('NOT intersected') 
            print(self.NAVIGATION_MANAGER.intersection_geometry.WorldTransform.value)

        else:
            #target = self.NAVIGATION_MANAGER.pick_result.WorldPosition.value
            self.NAVIGATION_MANAGER.update_ray_visualization()
            target = self.NAVIGATION_MANAGER.intersection_geometry.WorldTransform.value.get_translate()
            print('intersected')
            print(self.NAVIGATION_MANAGER.intersection_geometry.WorldTransform.value)
        ...
        if self.sf_pointer_button.value:
            _trans_vec = target * factor
            _input_mat = avango.gua.make_trans_mat(_trans_vec)
            
            #_head_trans_mat = avango.gua.make_trans_mat(self.NAVIGATION_MANAGER.get_head_matrix().get_translate())
            #_input_mat = self.NAVIGATION_MANAGER.transform_mat_in_ref_mat(_input_mat, _head_trans_mat)
...
class NavidgetNavigation(NavigationTechnique):
    ...
    def evaluate(self): # implement respective base-class function
        ...
        #self.NAVIGATION_MANAGER.set_navigation_matrix(_new_mat)
                ...
                #----- PLEASE DON'T ERASE -----
                #SPHERE_INTERSECT_GEOMETRY is to map the camera view on the sphere. This red dot should move on the sphere surface (like the rings in solar system)
                #proporcional to the ray movements. The sphere should be disatached from the instersection_geometry.
                #A good idea should be to rotate the sphere (it won't change) with the pointer_node and that rotation should be experienced by the Children (camera and sphere_intersection)
